...
async def incorrect_subject(message: Message):
    await message.answer(
        text="Выбери предмет из списка:", reply_markup=get_subject_kb()
    )


@router.callback_query(
    StateFilter(SolveTask.input_double_integral, SolveTask.input_extrem),
    F.data == "back",
)
...
async def incorrect_task(message: Message):
    await message.answer(text="Выбери задание из списка:", reply_markup=get_task_kb())
